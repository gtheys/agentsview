...
// ListMessageSourceUUIDs returns non-empty source UUIDs in message order.
func (s *BunStore) ListMessageSourceUUIDs(
	ctx context.Context, sessionID string,
) ([]string, error) {
	var uuids []string
	err := s.view(ctx, func(store bun.IDB) error {
		return store.NewSelect().Model((*bunmodel.Message)(nil)).
			Column("source_uuid").
			Where("session_id = ?", sessionID).
			Where("source_uuid <> ''").
			OrderExpr("ordinal ASC").
